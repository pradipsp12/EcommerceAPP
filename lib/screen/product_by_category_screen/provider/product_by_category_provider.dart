...
class ProductByCategoryProvider extends ChangeNotifier {
 ...
 filterInitialProductAndSubCategory(Category selectedCategory){
   ...
   filteredProduct = _dataProvider.products.where((element) => element.proCategoryId?.sId == selectedCategory.sId).toList();
   notifyListeners();
 }
 ...
 filterProductBySubCategory(SubCategory subCategory) {
   ...
   if (subCategory.name == 'All') {
     filteredProduct = _dataProvider.products.where((element) => element.proCategoryId?.sId == mySelectedCategory?.sId).toList();
   brands = [];
   } else {
     filteredProduct = 
  ...
  filterProductByBrand(){
    if(selectedBrands.isEmpty){
      filteredProduct = _dataProvider.products.where((element) => element.proSubCategoryId?.name == mySelectedSubCategory?.name).toList();
    }else{
