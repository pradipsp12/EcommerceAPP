  ...
  checkCoupon()async{
    try {
      ...
      if(response.isOk){
        ...
        SnackBarHelper.showErrorSnackBar('Coupon apply failed: ${response.body?['message'] ?? response.statusText}');
      }
      notifyListeners();
    } catch (e) {
      print(e);
      SnackBarHelper.showErrorSnackBar('Error something: $e');
      rethrow;
    }
  }
...
addOrder(BuildContext context)async{
  try {
    ...
    if(response.isOk) {
      ...
      SnackBarHelper.showErrorSnackBar('Order submission failed: ${response.body?['message'] ?? response.statusText}');

    }
  } catch (e) {
    print(e);
    SnackBarHelper.showErrorSnackBar('Error submitting order: $e');
    rethrow;
  }
}
  ...
  Future<void> stripePayment({required void Function() operation}) async {
    try {
      Map<String, dynamic> paymentData = {
        ...
        "amount":  getGrandTotal() * 100, // Amount in cents
        "currency": "usd",
  ...
  Future<void> razorpayPayment({required void Function() operation}) async {
    try {
      ...
      if (razorpayKey != null && razorpayKey != '') {
        var options = {
          'key': razorpayKey,
          'amount': getGrandTotal() * 100, // Amount in paise
          'name': "user",
          "currency": 'INR',
